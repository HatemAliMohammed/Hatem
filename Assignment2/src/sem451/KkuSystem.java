...
public class KkuSystem implements OptionalReserveTasks, FileNames, ReserveTasks {
	...
	public void showCLIMenu() {
		...
		OrderT TT=new OrderT();
		OrderD DD=new OrderD();
		...
		while(true) {
			...
			case "1":
				...
				TT.T();
				st=this.reserveBlock(new ReserveBlock(p,DD.getL(),TT.getT(),r));
				}
				catch(Exception e) {
			...
			case "6":
				...
				st=this.removeBlock(new ReserveBlock(null,DD.getL(),TT.getT(),r));
				break;
